# This is a edit of the original media-import anki addon.
# The problem is that importing video files like the original addon does ([sound: file.mp4])
# works, but the resulting cards will open, play, and close the video very quickly.
# I wanted a way to keep looking at the video after it ended, or to replay it how many times I needed.
# So the difference is that this new script puts the video embedded in html and use a bit of javascript
# so that pressing 'h' makes the video pause and play again.
...
# method to return the html and javascript code to add to the cards
def getHtmlJSCode(fname):
    return u'''<video controls="" autoplay="" loop="">
            <source src="%s" type="video/mp4">
            </video>
            <script>
              // make video play again when pressing the letter 'h'
              document.addEventListener("keypress", (e) => {
                let paused = document.querySelector("video").paused;
                if (e.key === "h") {
                  if (paused) {
